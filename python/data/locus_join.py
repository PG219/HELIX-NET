...
def build_atac_accessibility(
    ...
    # Auto-detect separator if peak_coords_sep is default or .csv vs .txt
    ...
    if meth_path:
        ...
        # Align patient_matrix rows to the matched methylation cohort
    ...
    # Schema contract: index must match cpg_anno.index, columns must match patient_barcodes
    ...
    for probe_id, m_row in matched_sub_df.iterrows():
        ...
        if peak_name in patient_matrix.columns:
            # patient_matrix: rows=patients, cols=peaks -> transpose to probe row
