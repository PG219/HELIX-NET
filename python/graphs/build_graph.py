...
def build_patient_graphs(use_atac: bool = USE_ATAC) -> list[Data]:
    ...
    constructed with (they also read USE_ATAC from config by default).
    """
    ...
    for patient_barcode in df_meth.index:
        ...
        if use_atac:
            # Extract ATAC accessibility for this patient
            atac_features = df_atac[patient_barcode].values
            # Concatenate features: [Nodes, 2]
            x = np.stack([meth_features, atac_features], axis=1)
        else:
            # Methylation-only: [Nodes, 1]
...
if __name__ == "__main__":
    print("Starting Phase 1/2 self-test...")
