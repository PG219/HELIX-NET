...
def generate_synthetic_data(seed: int = 42):
    ...
    meth_values = np.random.beta(a=2, b=5, size=(NUM_PATIENTS, NUM_CPG_SITES))
    ...
    cpg_pos = np.random.randint(1, 150_000_000, size=NUM_CPG_SITES)
    ...
    atac_values = np.clip(atac_values, 0, 1) # Keep in [0, 1] range for validity
