...
class CrossModalFusion(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # previously computed but discarded here; now exposed via self.last_attn_weights so a
        # caller (e.g. an interpretability script) can retrieve them after forward() without
        # changing the return signature everywhere it's called).
